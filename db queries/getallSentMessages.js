...
module.exports = async function getallSentMessages (req, res) {
    ...
    try{
        const [results, metadata] = await sequelize.query(`SELECT M.id,M.sender,M.reciver,M.message,M.subject, 
        M.createdAt,M.deleted ,U.id as senderId,U.email as senderEmail from messages as M
